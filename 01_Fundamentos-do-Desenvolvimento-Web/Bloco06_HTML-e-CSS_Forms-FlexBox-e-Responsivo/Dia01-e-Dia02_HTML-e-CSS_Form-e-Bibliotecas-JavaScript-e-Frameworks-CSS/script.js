function addStates() {
  ...
  for (let index = 0; index < nameStates.length; index += 1) {
    let currentState = nameStates[index];
    const allStates = document.createElement("option");
    allStates.innerHTML = currentState;
    allStates.value = acronyms[index];
    states.appendChild(allStates);
  }
}
addStates();
